...
**Additive drums must stay one hit per (voice, beat).** Because a fill layers rather than replaces, base and variation routinely name the same beat for the same voice — `pop`'s kick on 1 and `fill-every-4`'s kick on 1, `rock`'s snare on 4 and the fill's snare on 4. Every kit voice is a **single shared monophonic Tone synth** (`drumKit.ts`), so two starts at one instant make Tone's `Source.start` throw *"Start time must be strictly greater than previous start time"*, and that throw aborts the rest of the Transport tick, silently dropping the layers scheduled behind the drums. The humanizer cannot separate the copies either: its seed is derived from the beat, so both land on the bit-identical jittered time. `buildAllLayers` therefore merges the layered hits per `(voice, beat)` before scheduling, keeping the **louder** velocity so the fill's accents survive where they are stronger and the base's do where they are. `drumKit` additionally drops any hit whose time is not strictly later than that voice's previous one, so no future pattern can reach the assert. **[spec]**
