...
**Exception — minor-key dominant V (per-step quality pinning).** A major/dominant `V` is *not* diatonic in natural minor, so `minor-blues` and `andalusian` (both home `minor`) would have an unavailable `V` step. Verified empirically: `getDiatonicChord("V","minor",…)` is null, and today a `qualityOverride` does **not** rescue it (`resolveProgressionStep` gates "unavailable" before consulting the override). The decision is to add **per-step quality pinning** (see Section I) — a valid `qualityOverride` pins the chord on the degree's scale-position root regardless of diatonic availability — and to author those dominant V's as `V:7` in the DSL. The pin transposes with the key (verified: `V:7` ⇒ G7 in C minor, E7 in A minor).
...
The dominant V in `minor-blues` (DSL → `… V:7 … V:7`) and `andalusian` (`i VII VI V:7`) is authored with the `:7` quality pin so it resolves in natural minor (Section I). All other minor-category degrees are diatonic in natural minor.
...
### I. Per-step quality pinning (`resolveProgressionStep`)

Change `resolveProgressionStep` ([progressionDomain.ts:512](../../../src/progressions/progressionDomain.ts)) so a **valid `qualityOverride` pins the chord even when the degree is not diatonic** in the active scale, instead of returning "unavailable":

- Today: `if (!diatonic && !usingManualRoot) return { unavailable: true … }` runs *before* the override is consulted, so a non-diatonic degree with `qualityOverride` is wrongly unavailable.
- New: when `!diatonic && !usingManualRoot` but the override is valid (`qualityOverride` in `CHORD_DEFINITIONS`) **and** the degree's ordinal resolves to a scale note, pin: `root = getScaleNotes(rootNote, getProgressionHarmonyScaleName(scaleName))[ordinal]` (ordinal from the existing `getDegreeOrdinal`), `quality = qualityOverride`. Mark available; set `qualityOverrideApplied = true`, `diatonicQuality = null`.
- Still return `unavailable` only when there is no diatonic chord, no `manualRoot`, and no valid-override-with-resolvable-ordinal (e.g. a 7th degree in a 5-note scale).

This pins relative to the scale degree, so it transposes with the root (G7 in C minor, E7 in A minor). It is the mechanism that lets `minor-blues`/`andalusian` carry a dominant `V:7` in natural minor, and it also makes the manual quality-override UI work for borrowed degrees.
...
- **Quality pin (`progressionDomain.test.ts`):** `V` with `qualityOverride:"7"` in natural minor resolves to a dominant chord on the perfect-5th root (G7 in C, E7 in A) and is not `unavailable`; a degree whose ordinal exceeds the scale length (e.g. `VII` in a 5-note scale) with an override is still `unavailable`; a non-diatonic degree with **no** override remains `unavailable`.
...
- `src/progressions/progressionDomain.ts` — add `scale` to `ProgressionPreset`/`PRESET_SPECS` (incl. `V:7` pins in `minor-blues`/`andalusian`); per-step quality pin in `resolveProgressionStep`; load verbatim (drop ordinal-remap on load); remove now-unused availability helpers if no callers remain.
...
- Auto-adding 7ths/extensions to generated suggestions (suggestions stay plain triads in their current scale).
- Persisting suggestions as user presets.
- Reworking the static catalog's progression content, beyond (a) adding the `scale` field and (b) the `V:7` quality pins required for the minor-key dominant V.
