...
// ---------------------------------------------------------------------------
// cagedShapeDataAtom + cagedOctaveAtom (Plan G9)
// ---------------------------------------------------------------------------

describe("cagedShapeDataAtom + cagedOctaveAtom (Plan G9)", () => {
  beforeEach(() => {
    localStorage.clear();
  });

  it("renders ONLY one octave's polygon when single shape is active and cagedOctave=0", () => {
    const store = makeAtomStore([
    ...
    const data = store.get(shapeDataAtom);
    expect(data.shapePolygons.length).toBe(1);
  });

  it("renders ONLY the other octave's polygon when cagedOctave=1", () => {
    const store = makeAtomStore([
      [fingeringPatternAtom, "caged"],
      [rootNoteAtom, "C"],
      [scaleNameAtom, "Major"],
  ...
  it("low vs high produce DIFFERENT polygons (regression guard against octave being ignored)", () => {
    const storeLow = makeAtomStore([
      [fingeringPatternAtom, "caged"],
      [rootNoteAtom, "C"],
      [scaleNameAtom, "Major"],
      [cagedShapesAtom, new Set(["E"])],
    ...
    const low = storeLow.get(shapeDataAtom);
    const high = storeHigh.get(shapeDataAtom);
    expect(JSON.stringify(low.shapePolygons)).not.toBe(JSON.stringify(high.shapePolygons));
  });

  it("renders BOTH/ALL octaves when multiple shapes are active (octave atom ignored)", () => {
    const store = makeAtomStore([
      [fingeringPatternAtom, "caged"],
      [rootNoteAtom, "C"],
      [scaleNameAtom, "Major"],
      [cagedShapesAtom, new Set(["E", "D"])],
      [cagedOctaveAtom, 0],
    ]);
    const data = store.get(shapeDataAtom);
    // Each shape can have 1-2 octaves; combined > 1
    expect(data.shapePolygons.length).toBeGreaterThan(1);
  });

  it("falls back to first polygon when cagedOctave is out of range", () => {
    const store = makeAtomStore([
      [fingeringPatternAtom, "caged"],
      [rootNoteAtom, "C"],
      [scaleNameAtom, "Major"],
  ...
  it("highlightNotes only includes notes within the selected octave's fret bounds", () => {
    const storeLow = makeAtomStore([
      [fingeringPatternAtom, "caged"],
      [rootNoteAtom, "C"],
      [scaleNameAtom, "Major"],
    ...
    const low = storeLow.get(shapeDataAtom);
    const high = storeHigh.get(shapeDataAtom);
    // Notes should differ between octaves
    expect(new Set(low.highlightNotes)).not.toEqual(new Set(high.highlightNotes));
    // Each set's notes must lie within its respective bounds
    const lowBounds = low.boxBounds[0]!;
    for (const coord of low.highlightNotes) {
      const fret = parseInt(coord.split("-")[1], 10);
      expect(fret).toBeGreaterThanOrEqual(lowBounds.minFret);
      expect(fret).toBeLessThanOrEqual(lowBounds.maxFret);
    }
  });
});
