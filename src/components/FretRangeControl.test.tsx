...
describe('FretRangeControl', () => {
  ...
  describe('Start fret callbacks', () => {
    it('calls onStartChange with max(0, startFret - 1) on decrement', () => {
      ...
      render(
        <FretRangeControl {...defaultProps} startFret={3} onStartChange={onStartChange} layout="toolbar" />
      );
    ...
    it('calls onStartChange with min(endFret - 1, startFret + 1) on increment', () => {
      const onStartChange = vi.fn();
      render(
        <FretRangeControl {...defaultProps} startFret={3} endFret={12} onStartChange={onStartChange} layout="toolbar" />
      );
  ...
  describe('End fret callbacks', () => {
    it('calls onEndChange with max(startFret + 1, endFret - 1) on decrement', () => {
      ...
      render(
        <FretRangeControl {...defaultProps} startFret={3} endFret={12} onEndChange={onEndChange} layout="toolbar" />
      );
      const decrementBtns = screen.getAllByText('◀');
    ...
    it('calls onEndChange with min(maxFret, endFret + 1) on increment', () => {
      const onEndChange = vi.fn();
      render(
        <FretRangeControl {...defaultProps} startFret={3} endFret={12} maxFret={24} onEndChange={onEndChange} layout="toolbar" />
      );
      const incrementBtns = screen.getAllByText('▶');
  ...
  describe('Disabled states', () => {
    it('start decrement disabled when startFret === 0', () => {
      render(<FretRangeControl {...defaultProps} startFret={0} layout="toolbar" />);
      const decrementBtns = screen.getAllByText('◀');
      expect((decrementBtns[0] as HTMLButtonElement).disabled).toBe(true);
    });

    it('start increment disabled when startFret === endFret - 1', () => {
      render(<FretRangeControl {...defaultProps} startFret={11} endFret={12} layout="toolbar" />);
      const incrementBtns = screen.getAllByText('▶');
      expect((incrementBtns[0] as HTMLButtonElement).disabled).toBe(true);
    });

    it('end decrement disabled when endFret === startFret + 1', () => {
      render(<FretRangeControl {...defaultProps} startFret={3} endFret={4} layout="toolbar" />);
      const decrementBtns = screen.getAllByText('◀');
      expect((decrementBtns[1] as HTMLButtonElement).disabled).toBe(true);
    });

    it('end increment disabled when endFret === maxFret', () => {
      render(<FretRangeControl {...defaultProps} endFret={24} maxFret={24} layout="toolbar" />);
      const incrementBtns = screen.getAllByText('▶');
      expect((incrementBtns[1] as HTMLButtonElement).disabled).toBe(true);
    });
  });
