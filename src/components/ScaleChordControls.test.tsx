...
let defaultProps: React.ComponentProps<typeof ScaleChordControls>;
...
describe("ScaleChordControls", () => {
  ...
  it("renders Scale and Chord Overlay drawers", () => {
    ...
    expect(screen.getByText("Chord Overlay")).toBeInTheDocument();
  });

  it("shows chord-specific controls only when chordType is set", () => {
    const { rerender } = render(
      <ScaleChordControls {...defaultProps} chordType={null} />,
    );
    expect(
      screen.queryByText("Link chord root to scale"),
    ).not.toBeInTheDocument();

    rerender(<ScaleChordControls {...defaultProps} chordType="Major Triad" />);
    expect(screen.getByText("Link chord root to scale")).toBeInTheDocument();
  });
